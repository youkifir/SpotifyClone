...
// Скільки секунд аудіо на один рядок тексту (за замовчуванням)
...
export const FullScreenPlayer: React.FC = () => {
  ...
  const [lyricsStatus, setLyricsStatus] = useState<'idle' | 'loading' | 'found' | 'not_found' | 'error'>('idle')
  ...
  // Поточна секунда аудіо (0..30 для iTunes превью)
  const currentSec = currentTime.minute * 60 + currentTime.second

  // Парсимо повну тривалість пісні з поля duration треку (наприклад "3:54")
  // Для iTunes-треків duration = повна пісня, аудіо = 30 сек превью
  const trackDurationStr: string = (track as any).duration ?? '0:30'
  const durationParts = trackDurationStr.split(':').map(Number)
  const fullTrackSec = durationParts.length === 2
    ? durationParts[0] * 60 + durationParts[1]
    : 30

  const isItunes = (track as any).source === 'itunes'
  const previewSec = 30 // iTunes превью завжди 30 сек

  // Для iTunes: визначаємо з якого місця пісні починається превью.
  // Apple вирізає "найкращий" фрагмент — зазвичай приспів (~30-50% від початку).
  // Формула: previewOffset = (fullTrackSec - previewSec) / 2  → середина пісні.
  const previewOffsetSec = isItunes && fullTrackSec > previewSec
    ? (fullTrackSec - previewSec) / 2
    : 0

  // Стартовий рядок = той що відповідає previewOffset у повному тексті
  const startLine = totalLines > 0 && fullTrackSec > 0
    ? Math.round(totalLines * (previewOffsetSec / fullTrackSec))
    : 0

  // Скільки рядків тексту відповідає 30 секундам превью
  // (рівномірно розподіляємо превью на відповідний шматок тексту)
  const linesInPreview = isItunes && fullTrackSec > 0
    ? Math.round(totalLines * (previewSec / fullTrackSec))
    : totalLines

  // Секунд аудіо на один рядок — адаптивно під конкретну пісню
  const secsPerLine = linesInPreview > 0 ? previewSec / linesInPreview : SECS_PER_LINE

  // Поточний рядок = startLine + скільки рядків пройшло за поточний час аудіо
  const currentLineIndex = Math.min(
    startLine + Math.floor(currentSec / secsPerLine),
    totalLines - 1
  )

  // Автоскрол до активного рядка — тільки якщо користувач не скролить вручну
  useEffect(() => {
  ...
  // Після ручного скролу — через 4 сек відновлюємо авто-скрол
  ...
  const fetchLyrics = useCallback(async (trackId: string | number) => {
    if (track.lyrics) {
  ...
  }, [isFullScreen, track?.id, lastTrackId, fetchLyrics])
  ...
  const trackImageUrl = track.image?.startsWith('http') ? track.image : `${API}/${track.image}`

  // Будуємо масив рядків для рендеру, відстежуючи індекс серед непустих
  let nonEmptyCount = 0
  const displayLines = lyricsLines.map((line, i) => {
    ...
    <div className="fixed inset-0 bg-gradient-to-b from-[#1a1a1a] to-black z-50 flex flex-col p-4 sm:p-6 md:p-12 text-white transition-all duration-300">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 flex-1 max-w-6xl mx-auto w-full overflow-hidden min-h-0">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left gap-4 sm:gap-6 w-full md:w-2/5 shrink-0">
          ...
          <div className="min-w-0 w-full">
            ...
            <p className="text-neutral-400 text-sm sm:text-base mt-1 line-clamp-1">{(track as any).artist || track.desc?.slice(0, 40)}</p>
          </div>
        </div>
        ...
        <div className="flex-1 w-full h-full min-h-0 flex flex-col">
          ...
          >
            ...
            {lyricsStatus === 'found' && lyricsLines.length > 0 && (
            ...
            {(lyricsStatus === 'not_found' || lyricsStatus === 'error') && (
              <div className="flex flex-col items-start gap-3 mt-4">
                ...
                <p className="text-sm text-neutral-600 mt-1">
                  {lyricsStatus === 'error'
                    ? 'Помилка при завантаженні. Спробуй пізніше.'
      ...
      <div className="w-full max-w-2xl mx-auto flex flex-col items-center gap-3 pt-6 shrink-0">
        ...
        <div className="flex items-center gap-3 w-full max-w-xs">
          {/* Іконка гучності — клік мютить/розмютить */}
          <button
