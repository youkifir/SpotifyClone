...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen = false, onClose, collapsed = false, onToggleCollapse }) => {
  ...
  // collapse живе тут — не треба пропсів
  ...
  // Кнопка-стрілка (однакова логіка, різний напрямок)
  const CollapseBtn = ({ dir }: { dir: 'left' | 'right' }) => (
    <button
      onClick={() => setCollapsed(v => !v)}
      aria-label={dir === 'left' ? 'Згорнути' : 'Розгорнути'}
      title={dir === 'left' ? 'Згорнути бібліотеку' : 'Розгорнути бібліотеку'}
      style={{ display: 'flex' }}
      className="w-7 h-7 rounded-full items-center justify-center text-neutral-400 hover:text-white hover:bg-[#2a2a2a] hover:scale-110 transition shrink-0"
    >
    ...
    <>
      ...
      >
        <div className="bg-[#121212] h-full lg:rounded-lg p-2 flex flex-col gap-2 overflow-hidden">
            ...
            {!token ? (
              ...
              <>
                <div className="flex items-center gap-3 min-w-0">
                  <img className="w-8 shrink-0" src={assets.stack_icon} alt="Library" />
                  <p className="font-semibold truncate">{t('yourLibrary')}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  {/* Стрілка вліво — тільки на десктопі */}
                  <span className="hidden lg:block">
                    <CollapseBtn dir="left" />
                  </span>
                  <img
                    onClick={() => setIsModalOpen(true)}
                    className="w-5 cursor-pointer opacity-70 hover:opacity-100 hover:scale-110 transition"
                    ...
                    title="Створити плейлист"
                  />
                  {/* Хрестик — тільки мобільний */}
                  <button
                    onClick={onClose}
                    aria-label="Закрити"
                    className="lg:hidden w-6 h-6 flex items-center justify-center text-neutral-400 hover:text-white transition text-lg"
                  >✕</button>
                </div>
              </>
            )}
          ...
          {/* Список плейлистів — ховаємо при collapsed */}
          {!collapsed && (
              ...
              {!token ? (
                ...
                <div className="p-4 bg-[#242424] hover:bg-[#2a2a2a] transition-colors rounded-lg flex flex-col items-start gap-1">
                  ...
                  <button
                    onClick={() => setIsModalOpen(true)}
                    className="bg-white text-black text-sm font-bold px-4 py-1.5 rounded-full mt-4 hover:scale-105 hover:bg-neutral-200 transition"
                  >{t('createPlaylist')}</button>
                </div>
              ) : (
                <>
                  <div className="flex flex-col gap-1">
                    {playlists.map((p) => <PlaylistItem key={p._id} playlist={p} />)}
                  </div>
                  {sharedPlaylists.length > 0 && (
                    <div className="flex flex-col gap-1 mt-2">
                      ...
                      {sharedPlaylists.map((p) => <PlaylistItem key={p._id} playlist={p} isShared />)}
                    </div>
                  )}
                </>
              )}
